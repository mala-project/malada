...
class DFTConvergenceProvider(Provider):
    ...
    def __write_to_xml(self):
        ...
        cpnode = SubElement(
            calculationparameters, "element", {"type": "string"}
        )
    ...
    def __check_input_correctness(self, atoms):
        ...
        if (
            ...
            if atoms.get_chemical_symbols()[0] != self.parameters.element:
                raise Exception(
                    "Mismatch between user input and provided file."
                )
    ...
    def __create_dft_convergence_inputs(
        ...
        if parameters_to_converge == "kpoints":
            if try_number == 0:
                ...
                spacing = (
                    kpoints_guesses[self.parameters.element][-1]
            ...
            if self.parameters.dft_calculator == "qe":
                if try_number == 0:
                    ...
                    spacing = (
                        cutoff_guesses_qe[self.parameters.element][-1]
                ...
                if try_number == 0:
                    converge_list = cutoff_guesses_vasp[
                        self.parameters.element
                    ]
                else:
                    spacing = (
                        cutoff_guesses_vasp[self.parameters.element][-1]
        ...
        for entry in converge_list:
            ...
            if self.parameters.dft_calculator == "qe":
                ase.io.write(
                    this_folder + self.parameters.element + ".pw.scf.in",
                    atoms_Angstrom,
                    "espresso-in",
