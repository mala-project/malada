...
class DFTConvergenceProvider(Provider):
    ...
    def __init__(
        self,
        parameters,
        external_convergence_results=None,
        external_convergence_folder=None,
        predefined_kgrid=None,
        predefined_cutoff=None,
    ):
    ...
    def provide(
        self, provider_path, supercell_file, create_submit_script=True
    ):
        """
        ...
        if self.external_convergence_results is None:
            if self.external_convergence_folder is None:
                ...
                kpoint_try = 0
                while (
                    self.converged_cutoff is None
                    and cutoff_try < self.parameters.maximum_cutoff_try
                ):

                    # First, we create the inputs.
                    cutoff_folders = self.__create_dft_convergence_inputs(
                        "cutoff", supercell_file, provider_path, cutoff_try
                    )

                    # Then we run.
                    for cutoff_folder in cutoff_folders:
                        print("Running DFT in", cutoff_folder)
                        dft_runner.run_folder(cutoff_folder, "dft")

                    # Afterwards, we check if running was succesful.
                    if self.__check_run_success(
                        provider_path, "cutoff", fixed_kpoints=(1, 1, 1)
                    ):
                        self.converged_cutoff = (
                            self.__analyze_convergence_runs(
                                provider_path,
                                "cutoff",
                                supercell_file,
                                fixed_kpoints=(1, 1, 1),
                        ...
                        if self.parameters.run_system == "slurm_creator":
                            all_submit_file = open(
                                os.path.join(provider_path, "submit_all.sh"),
                                mode="w",
                            )
                            all_submit_file.write("#!/bin/bash\n\n")
                            for cutoff_folder in cutoff_folders:
                                all_submit_file.write(
                                    "cd " + cutoff_folder.split("/")[-2] + "\n"
                                )
                    ...
                    if self.converged_cutoff is None:
                        print(
                            "Could not find an aedaquate cutoff energy, "
                            "trying again with larger cutoff energies. "
                            "This will be try nr. " + str(cutoff_try + 2)
                        )
                    cutoff_try += 1
                ...
                # Create, run and analyze.
                while (
                    self.converged_kgrid is None
                    and kpoint_try < self.parameters.maximum_kpoint_try
                ):
                    kpoints_folders = self.__create_dft_convergence_inputs(
                        "kpoints", supercell_file, provider_path, kpoint_try
                    )
                    for kpoint_folder in kpoints_folders:
                        print("Running DFT in", kpoint_folder)
                        dft_runner.run_folder(kpoint_folder, "dft")
                    ...
                    if self.converged_kgrid is None:
                        print(
                            "Could not find an aedaquate k-grid, trying again"
                ...
                # First: cutoffs.
                if self.converged_cutoff is None:
                    self.converged_cutoff = self.__analyze_convergence_runs(
                        self.external_convergence_folder,
                        "cutoff",
                        supercell_file,
                        fixed_kpoints=(1, 1, 1),
                    )
                if self.converged_cutoff is None:
                    raise Exception(
                        "Provided convergence data not sufficient,"
            ...
            copyfile(
                self.external_convergence_results,
    ...
    def __write_to_xml(self):
        if isinstance(self.parameters.element, str):
            element_list = [self.parameters.element]
        else:
            element_list = self.parameters.element
        ...
        cpnode = SubElement(
            calculationparameters, "crystal_structure", {"type": "string"}
        )
        cpnode.text = self.parameters.crystal_structure
        cpnode = SubElement(
            calculationparameters, "temperature", {"type": "float"}
        )
        cpnode.text = str(self.parameters.temperature)
        cpnode = SubElement(
            calculationparameters, "dft_calculator", {"type": "string"}
        )
    ...
    def __check_input_correctness(self, atoms):
        ...
        # user input.)
        if (
            self.parameters.element is not None
            and self.parameters.number_of_atoms is not None
        ):
            # if not np.all(
            #     atoms.get_atomic_numbers() == atoms.get_atomic_numbers()
            # ):
            #     raise Exception(
    ...
    def __create_dft_convergence_inputs(
        self, parameters_to_converge, posfile, working_directory, try_number
    ):
        ...
        if parameters_to_converge == "kpoints":
            if try_number == 0:
                for grids in kpoints_guesses[element_string]:
                    converge_list.append(
                        self.__k_edge_length_to_grid(grids, atoms_Angstrom)
                    )
            else:
                spacing = (
                    kpoints_guesses[element_string][-1]
                    - kpoints_guesses[element_string][-2]
                )
                converge_list.append(
                    self.__k_edge_length_to_grid(
                        kpoints_guesses[element_string][-1]
                        + spacing * (try_number - 1) * 2
                        + 1 * spacing,
                        atoms_Angstrom,
                    )
                )
            ...
            if self.parameters.dft_calculator == "qe":
                if try_number == 0:
                    converge_list = cutoff_guesses_qe[element_string]
                else:
                    spacing = (
                        cutoff_guesses_qe[element_string][-1]
                        - cutoff_guesses_qe[element_string][-2]
                    )
                    converge_list.append(
                        cutoff_guesses_qe[element_string][-1]
                        + spacing * (try_number - 1) * 2
                ...
                if try_number == 0:
                    converge_list = cutoff_guesses_vasp[element_string]
                else:
                    spacing = (
                        cutoff_guesses_vasp[element_string][-1]
                        - cutoff_guesses_vasp[element_string][-2]
                    )
                    converge_list.append(
                        cutoff_guesses_vasp[element_string][-1]
                        + spacing * (try_number - 1) * 2
        ...
        for entry in converge_list:
            if parameters_to_converge == "kpoints":
                ...
                cutoff = entry
            this_folder = self.__make_convergence_folder(
                kpoints, cutoff, working_directory
            )
            ...
            if self.parameters.dft_calculator == "qe":
                ase.io.write(
                    this_folder + element_string + ".pw.scf.in",
                    atoms_Angstrom,
                    "espresso-in",
    ...
    def __check_run_success(
        ...
        if parameter_to_converge == "cutoff":
            ...
            if self.parameters.dft_calculator == "qe":
                converge_list = glob.glob(
                    os.path.join(base_folder, str(fixed_cutoff) + "Ry_k*")
                )
            elif self.parameters.dft_calculator == "vasp":
                converge_list = glob.glob(
                    os.path.join(base_folder, str(fixed_cutoff) + "eV_k*")
                )
        else:
        ...
        for entry in converge_list:
            if self.parameters.dft_calculator == "qe":
                convergence_file_lists = glob.glob(
                    os.path.join(entry, "*.out")
                )
    ...
    def __analyze_convergence_runs(
        self,
        base_folder,
        parameter_to_converge,
        supercell_file,
        ...
        for entry in converge_list:
                ...
                if len(k_argument) == 3:
                    ...
                    if len(k_argument) == 6:
                        ...
                        if scaling_y != 1:
                            if scaling_x == 1:
                                argument_y = int(
                                    (os.path.basename(entry).split("k")[1])[
                                        1:3
                                    ]
                                )
                            else:
            ...
            if self.parameters.dft_calculator == "qe":
                ...
                energy = self.__get_vasp_energy(os.path.join(entry, "OUTCAR"))
        ...
        for i in range(1, len(result_list)):
            ...
            if (
                np.abs(result_list[i][1] - result_list[i - 1][1])
                < self.parameters.dft_conv_accuracy_meVperatom
            ):
    ...
    def __get_qe_energy(self, file):
        ...
        with open(file, "r") as f:
            ...
            for l in ll:
                if (
                    "total energy" in l
                    and "is F=E-TS" not in l
                    and "is the sum of" not in l
                ):
                    energy = float((l.split("=")[1]).split("Ry")[0])
                if (
                    "convergence NOT achieved" in l
                    or "oom-kill" in l
                    or "CANCELLED" in l
                    or "BAD TERMINATION" in l
                ):
        ...
        return float(
            (energy * Rydberg * 1000) / self.parameters.number_of_atoms
        )
