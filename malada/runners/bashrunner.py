    ...
    def run_folder(self, folder, calculation_type):
        ...
        if calculator_type == "qe":
            if calculation_type == "dft":
                ...
                run_process = subprocess.Popen("pw.x -in "+filename+" > "+job_name+".out", cwd=folder, shell=True)
                run_process.wait()
            if calculation_type == "md":
                ...
                if len(scf_file) != 1 or \
                   ...
                   len(dos_file) != 1:
                    ...
                    raise Exception("Run folder with ambigous content.")
                run_process = subprocess.Popen("pw.x -in "+os.path.basename(scf_file[0])+" > "+job_name+".out;" +
                                               "pp.x -in "+os.path.basename(dens_file[0])+";" +
                                               "dos.x -in "+os.path.basename(dos_file[0])+";" +
                                               "pp.x -in "+os.path.basename(ldos_file[0])+";"
